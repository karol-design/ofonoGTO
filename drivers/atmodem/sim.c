...
static const char *ztev_zrap_prefix[] = { "+ZRAP:", NULL };
...
static void at_sind_euiccid_read_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	struct cb_data *cbd = user_data;
	GAtResultIter iter;
	ofono_sim_euiccid_cb_t cb = cbd->cb;
	struct ofono_error error;
	const char *euccid;
	...
	g_at_result_iter_init(&iter, result);
	if (!g_at_result_iter_next(&iter, "^SIND: euiccid,"))
		goto error;
	...
	if (!g_at_result_iter_next_string(&iter, &euccid))
		goto error;
...
static void at_read_euiccid(struct ofono_sim *sim, ofono_sim_euiccid_cb_t cb,
				void *data)
{
	struct sim_data *sd = ofono_sim_get_data(sim);
	struct cb_data *cbd = cb_data_new(cb, data);

	if (g_at_chat_send(sd->chat, "AT^SIND=\"euiccid\",2", sind_prefix,
				at_sind_euiccid_read_cb, cbd, g_free) > 0)
		return;
...
static void gemalto_spic_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	struct cb_data *cbd = user_data;
	struct ofono_sim *sim = cbd->user;
	ofono_sim_pin_retries_cb_t cb = cbd->cb;
...
static void ztev_zrap_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	struct cb_data *cbd = user_data;
	ofono_sim_pin_retries_cb_t cb = cbd->cb;
	const char *final = g_at_result_final_response(result);
	...
	if (!g_at_result_iter_next(&iter, "+ZRAP:"))
		goto error;

	BUILD_PIN_RETRIES_ARRAY(password_types, ARRAY_SIZE(password_types), retries);
					...
					void *data)
{
	...
	case OFONO_VENDOR_ZTE_VANILLA:
		if (g_at_chat_send(sd->chat, "AT+ZRAP?", ztev_zrap_prefix,
					ztev_zrap_cb, cbd, g_free) > 0)
			return;
		break;
	default:
